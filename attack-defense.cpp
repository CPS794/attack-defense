...
const int destroyed[13][13] = {
	...
	{4,1,2,7,8,3,4,5,6,9,10,11,12},
	{6,1,2,3,7,8,9,4,5,6,10,11,12},
	{8,1,2,3,4,7,8,9,10,5,6,11,12},
	{8,1,2,3,6,7,8,9,12,4,5,10,11},
	{6,1,2,6,7,8,12,3,4,5,9,10,11},
	{2,2,8,1,3,4,5,6,7,9,10,11,12},
	{2,3,9,1,2,4,5,6,7,8,10,11,12},
	{4,3,4,9,10,1,2,5,6,7,8,11,12},
	{8,3,4,5,6,9,10,11,12,1,2,7,8},
	{4,3,6,9,12,1,2,4,5,7,8,10,11},
...
double w(double p, double t) { // w+: t=chi w-:t=delta
	return pow(p,t)/pow(pow(p,t)+pow(1-p,t),1.0/t);
}
...
struct Attack
{
	...
	void calculate(int cd, int ca, double g, double l, double lambda, double chi, double delta) {
		...
		{
			double tmp = 1.0;
			for (int j = 1; j <= destroyed[i][0]; ++j)
			{
				tmp *= pAB[destroyed[i][j]];
			}
			if (destroyed[i][0] == 0)
			{
				tmp = 0;
